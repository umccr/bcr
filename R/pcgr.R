...
#'
#' \dontrun{
#' f1 <- "~/projects/woof/woof_compare_reports/nogit/umccrise_2.3.0rc.4/umccrised/SEQC50__SEQC-II_tumour/small_variants/SEQC50__SEQC-II_tumour-somatic.pcgr.snvs_indels.tiers.tsv"
...
pcgr_cmp <- function(f1, f2) {
  ...
  col_sel <- c("GENOMIC_CHANGE", "TIER", "SYMBOL")
  ...
  diff1 <- dplyr::anti_join(p1, p2, by = col_sel) |>
    ...
    dplyr::ungroup() |>
    dplyr::select("GENOMIC_CHANGE", "symbol_same", "tier_same", "SYMBOL.run1", "SYMBOL.run2", "TIER.run1", "TIER.run2")
