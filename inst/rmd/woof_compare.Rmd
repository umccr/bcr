...
snv_res %>%
  ...
  DT::datatable(
    ...
    options = list(scroller = TRUE, scrollY = snv_tab_params$scroll_y, scrollX = TRUE,
                   autoWidth = TRUE, keys = TRUE, dom = 'Blfrtip')) %>%
  DT::formatStyle(
    paste0(c("SNP_", "IND_"), rep(c("Rec", "Prec", "f1"), each = 2)),
    background = styleColorBar(snv_tab_params$bar_range, color = snv_tab_params$bar_colour),
    backgroundSize = snv_tab_params$bgsize, backgroundRepeat = 'no-repeat', backgroundPosition = 'center') %>%
  DT::formatCurrency(snv_tab_params$big_mark_cols, currency = "", interval = 3, mark = ",", digits = 0)
```
...
sv_eval_res %>%
  ...
  DT::datatable(
    ...
    filter = list(position = "top", clear = FALSE, plain = FALSE),
    extensions = c("Scroller", "KeyTable"),
    options = list(scroller = TRUE, scrollX = TRUE, scrollY = sv_tab_params$scroll_y)) %>%
  DT::formatStyle(
    c("Recall", "Precision"),
    background = DT::styleColorBar(sv_tab_params$bar_range, color = sv_tab_params$bar_colour),
    backgroundSize = sv_tab_params$bgsize, backgroundRepeat = 'no-repeat', backgroundPosition = 'center') %>%
  DT::formatCurrency(sv_tab_params$big_mark_cols, currency = "", interval = 3, mark = ",", digits = 0)
```
...
sv_fpfn_res %>%
  ...
  DT::datatable(
    ...
    options = list(scroller = TRUE, scrollX = TRUE,
                   scrollY = sv_fpfn_tab_params$scroll_y)) %>%
  DT::formatStyle("FP_or_FN",
                  backgroundColor = DT::styleEqual(c("fp", "fn"),
                                               c("#66FF66", "#FF6666"))) %>%
  DT::formatCurrency(sv_fpfn_tab_params$big_mark_cols,
...
```{r circos_display, fig.width=5, fig.height=5, results="asis"}
x <- sv_circos_res %>%
